...
NetBlock=$(route -n| grep ens33| grep -v '^0'| grep -v '^169.254'| awk '{print $1}')
NetName=$(getent networks $NetBlock| awk '{print $1}')

#Router variables
RoutIP=$(ip route| grep default| awk '{print $3}')
RoutName=$(getent hosts $RoutIP| awk '{print $2}')
