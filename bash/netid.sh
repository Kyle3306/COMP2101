...
while [ $# -gt 0 ]; do
  case "$1" in
  -v)
  #we only land if if -v is present on the command line
    verbose="yes"
    ;;
   *)
   #we are only here if the user put something else on the command line
    #exit with an error message if the interface variable isn't empty
    if [ "$interface" = '' ]; then
      interface=$1
    else
      echo "Invalid input"
...
# define the interface being summarized if it isn't already defined
# if interface is empty, run the below command
if [ "$interface" = '' ]; then
  interfaces=$(ip a |awk '/: e/{gsub(/:/,"");print $2}'| sed -e s/@ens34//g)
else
